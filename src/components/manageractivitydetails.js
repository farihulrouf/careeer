...
class managerUserManagementTable extends Component {
    ...
    getassessmentlist = () => {
        ...
        Api.getloginactivityetailsmanager(orgId, managerId, token).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
    ...
    searchInputHandler = (event) => {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getActivitylistsManagerSearch(orgId, managerId, token, this.state.month, searchstring).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
    ...
    handleChange = (value) => {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getActivitylistsManagerSearch(orgId, managerId, token, value, searchstring).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
    ...
    fromToDate =(e)=> {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getActivitylistsmanagerSearchByDate(orgId, managerId, token, startDate, endDate).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
                ...
                <div style={{ height: "480px", overflowY: "auto" }}>
                    ...
                    >
                        <tr style={{ display: "flex", alignItems: "center", fontSize: "16px", height: "50px", backgroundColor: "#f8f8f8" }}>
                            ...
                            <td style={styles.date}><b style={{ marginLeft: "-39px" }}>Department</b></td>
                            <td style={styles.department}><b style={{ marginLeft: "-32px" }}>Email</b></td>
                            <td style={styles.score}><b style={{ marginLeft: "-26px" }}>Login Date</b></td>
                        ...
                        {this.state.totolassesmentlist.map((ele, index) => (
                            <tr>
                                <td style={styles.mainContainer} className="table">
                                    ...
                                    <div style={styles.department}>
                                        <td style={{ flex: 0.9, display: "flex", alignItems: "center" }}>
                                            ...
                                            >
                                                ...
                                                >
                                                    <DisplayPicture profile={ele.profilePicture == "" ? this.props.profilePicture : "https://dk91y438j4an1.cloudfront.net/" + ele.profilePicture ? "https://dk91y438j4an1.cloudfront.net/" + ele.profilePicture : this.props.profilePicture} />
                                    ...
                                    {/* <div style={styles.department}>{ele.phone}</div> */}
                                    {/* <div style={styles.score}>{timeStampToDateTime(ele.loggedUser)}</div> */}
                                    <td style={styles.score}>{moment(ele.loggedUser).format('MM/DD/YYYY')}</td>
                                    <td style={styles.score}>{moment(ele.loggedUser).format('h:mm:ss a')}</td>
