...
class managerUserManagementTable extends Component {
    ...
    getassessmentlist = () => {
        ...
        Api.getassessmentlistdetsilsmanager(orgId, managerId, token).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
                    // document.message.success("something went wrong");
                    this.setState({
    ...
    searchInputHandler = (event) => {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getAssessmentlistsManagerSearch(orgId, managerId, token, this.state.month, searchstring).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
    ...
    handleChange = (value) => {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getAssessmentlistsManagerSearch(orgId, managerId, token, value, searchstring).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
    ...
    fromToDate =(e)=> {
        ...
        let managerId = decryptData(localStorage.employeeId);
        let token = decryptData(localStorage.token);
        Api.getAssessmentlistsManagerSearchByDate(orgId, managerId, token, startDate, endDate).then(
            res => {
                console.log(res, "res")
                if (res.status == 200) {
                    console.log(res.data.data, "ddaa")
        ...
        return (
            ...
            <div style={{ background: "#F8F8F8", padding: "10px 30px" }}>
                {/* <div className="userManagementContainer">
                    <div className="title subhead">Assessmenet Management</div>
                </div> */}
